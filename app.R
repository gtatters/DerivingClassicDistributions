...
server <- function(input, output, session) {
    ...
    if (input$dist == "t") {
      ...
      if (!is.null(prev_vals)) {
        hist(prev_vals, probability = TRUE, breaks = breaks,
             ...
             xlab = "F statistic", xlim = c(0, 10))
      }
      
      hist(f_vals, probability = TRUE, breaks = breaks,
           col = "#569BBD", border = "black",
           add = !is.null(prev_vals), xlim = c(0, 10))
      ...
      if (!is.null(prev_vals)) {
        hist(prev_vals, probability = TRUE, breaks = breaks,
             col = rgb(0.7, 0.7, 0.7, 0.4), border = NA,
             main = paste("Chi-square distribution (df =", input$k_chi, ")"),
  ...
  output$explain <- renderText({
    ...
    if (input$dist == "t") {
      paste0(
        "t distribution: emerges from drawing n samples from a normal distribution\nand standardizing sample means: (mean/sd/sqrt(n_t)).\n",
        "Red line shows the theoretical t distribution densities for comparison.\n",
      ...
      paste0(
        ...
        "Red lines show the theoretical t and F distribution densities for comparison.\n",
        "Used extensively in ANOVA and variance comparison."
      )
      
    } else {
      paste0(
        "Chi-square distribution: emerges from summing squared standard normal variables.\n",
        "If Z ~ N(0,1), then χ²(k) = sum(Z^2) over k independent draws.\n",
        "Here, we simulate k standard normals and square + sum them.\n",
        "This connects directly to variance estimation and likelihood theory.\n"
      )
    }
  })
}
